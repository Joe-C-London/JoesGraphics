...
class BattlefieldScreenTest {
    ...
    fun testBattlefieldNoVotes() {
        ...
        assertPublishes(
            ...
            LIB WOULD HAVE MAJORITY ON NO ADVANCES
            """.trimIndent(),
        )
    }
    ...
    @Test
    fun testBattlefieldResultNoMajorityLines() {
        val pc = Party("Progressive Conservative", "PC", Color.BLUE)
        val lib = Party("Liberal", "LIB", Color.RED)
        val grn = Party("Green", "GRN", Color.GREEN.darker())
        val ndp = Party("New Democratic Party", "NDP", Color.ORANGE)
        ...
        val screen = BattlefieldScreen.build(
            ...
            majorityLines = false.asOneTimePublisher(),
            title = "PRINCE EDWARD ISLAND".asOneTimePublisher(),
        )
        screen.setSize(1024, 512)
        compareRendering("BattlefieldScreen", "Result-NoMajority", screen)
        assertPublishes(
            screen.altText.map { it(1000) },
            """
            PRINCE EDWARD ISLAND
